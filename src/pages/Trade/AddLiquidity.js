...
import {
	...
	useMintState, useTransactionAdder,
	useUserDeadline,
		...
		return noLiquidity ? (
			...
			<AutoColumn gap="20px">
				<RowFlat style={{ marginTop: "20px", marginBottom: "32px", alignItems: "center" }}>
					<span className={'text-5xl leading-none mr-3 text-white'}>{liquidityMinted?.toSignificant(6)}</span>
					<DoubleCurrencyLogo
						currency0={currencies[Field.CURRENCY_A]}
						currency1={currencies[Field.CURRENCY_B]}
						size={30}
						...
						<ColumnCenter>
							<div className={"p-4 rounded-lg mb-4 bg-fadeBlack mt-8 w-full"}>
								<AutoColumn gap="12px">
									...
									<span className={'text-white'}>The ratio of tokens you add will set the price of this pool.</span>
									<span className={'text-white'}>Once you are happy with the rate click supply to review.</span>
								</AutoColumn>
							</div>
						</ColumnCenter>
					)}
