...
const MarketHistory = ({
  ...
  const apy = useMemo(() => {
    if (
      ...
      !(selectedAsset?.supplyApy instanceof BigNumber) ||
  ...
  return (
    <div className="bg-fadeBlack py-4 px-6 mt-5 rounded-lg">
      <div className="flex flex-row items-center space-x-4">
        ...
        <Select options={options} onChange={changeCurrentSymbol} />
      </div>
      <div className="bg-black p-6 mt-4 rounded-xl">
        <div className="flex flex-col space-y-8 lg:space-y-0 lg:flex-row items-stretch lg:items-center justify-between">
          <div className="flex space-x-6 text-white">
            {currentAsset !== 'bnb' && (
              <div
                ...
                onClick={() =>
                  addToken(currentAsset, settings.decimals[currentAsset || 'usdc']?.token, 'token')
                }
              >
                <span>{currentAsset?.toUpperCase()}</span>
                <img
                  src={plusButtonIcon}
                  alt="plusButtonIcon"
            ...
            <div
              ...
              onClick={() =>
                addToken(currentAsset, settings.decimals[currentAsset || 'usdc']?.atoken, 'atoken')
              }
            >
              <span>{`a${currentAsset === 'btcb' ? 'BTC' : currentAsset?.toUpperCase()}`}</span>
              <img
                src={plusButtonIcon}
                alt="plusButtonIcon"
                className="ml-2 inline cursor-pointer"
          ...
          <div className="text-white flex flex-col lg:flex-row items-stretch lg:items-center space-y-3 lg:space-y-0 lg:space-x-12">
            <div className="flex flex-row lg:flex-col items-center justify-between lg:justify-center lg:space-y-1">
              <div className="tooltip relative">
                <div className="text-base lg:text-center tooltip-label">Net APY</div>
                <span className="label">
                  This rate blends the Supply or Borrow APY with the ANN Distribution APY based on
                  the current price of ANN
                </span>
              </div>
            ...
            <div className="flex flex-row lg:flex-col items-center justify-between lg:justify-center lg:space-y-1 relative pl-8 lg:pl-5">
              ...
              <div className="font-bold text-lg text-right lg:text-center">
                {`${new BigNumber(
                  activeType === TYPES.Supply ? selectedAsset?.supplyApy : selectedAsset?.borrowApy,
                )
                  ?.dp(2, 1)
                  ?.toString(10)}%`}
              </div>
            ...
            <div className="flex flex-row lg:flex-col items-center justify-between lg:justify-center lg:space-y-1 relative pl-8 lg:pl-5">
              <div className="flex flex-row items-center space-x-2">
                <div className="absolute left-0">
                  <img alt={'Annex'} src={Annex} style={{ width: 20, height: 20 }} />
                </div>
            ...
            <div className="flex flex-row lg:flex-col items-center justify-between lg:justify-center lg:space-y-1">
              <div className="text-base lg:text-center">
                Total {activeType === TYPES.Supply ? 'Supply' : 'Borrow'}
              </div>
        ...
        <MarketHistoryChart data={data} withANN={withANN} type={activeType} />
          ...
          <div className="flex flex-col text-white w-full">
            ...
            <div className="flex justify-between px-4 rounded-md py-1 items-center transition-all hover:bg-fadeBlack">
              ...
              <div className="font-medium text-right">
                ...
                ) : Number(marketInfo?.supplierCount) >= 0 ? (
            ...
            <div className="flex justify-between px-4 rounded-md py-1 items-center transition-all hover:bg-fadeBlack">
              ...
              <div className="font-medium text-right">
                ...
                ) : Number(marketInfo?.borrowerCount) >= 0 ? (
                      ...
                      new BigNumber(1)
                        .div(
                          new BigNumber(marketInfo.exchangeRate).div(
                            new BigNumber(10).pow(
                              18 +
                                +parseInt(settings.decimals[currentAsset || 'usdc']?.token, 10) -
                                +parseInt(settings.decimals[currentAsset || 'usdc']?.atoken, 10),
                            ),
                          ),
                        )
                        .toFixed(6) || '-'
                    )}{' '}
                    <span className="text-primaryLight">{marketInfo.symbol || ''}</span>
                  </>
                ) : (
                  '-'
                )}
